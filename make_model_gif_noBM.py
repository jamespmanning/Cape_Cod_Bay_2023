...
include_temp_obs='no' # 'yes' overlays positions of observed bottom temps
...
def getgbox(area):
  ...
  if area=='SNE':
    ...
    gbox=[-73.,-67.,37.,42.] # for SNw shelf west  
  elif area=='OOI':
...
def get_wind_ncep_2(starttime,endtime,lat,lon): # added Sep 2023
    ...
    if starttime<datetime.now()-timedelta(days=60):# get reanalysis
        ...
        df=pd.read_csv('/home/user/drift/data/ncep_global_4a77_8730_d053.csv') # starts 4/8/2025
        ...
        #t=pd.to_datetime(df['time (UTC)'],format='%Y-%m-%dT%H:%M:%S').values
        t=pd.to_datetime(df['time (UTC)'],format='mixed').values
        u_wind,v_wind=[],[]
...
def plotit(lons,lats,slons,slats,stemp,temp,depth,time_str,path_save,dpi=80,area='OOI',clevs=[39.,44.,0.5],lat_w=42.9,lon_w=-70.3,wind=pd.DataFrame([[datetime.now()-timedelta(1),datetime.now()],[.1,.1],[.1,.1]]),dtime=dtime,model='DOPPIO',detide='n'):
    ...
    if include_wind=='yes':
       ...
       #idex=wind.index.get_loc(datetime.strptime(time_str[0:16],"%Y-%m-%d %H:%M"))#,method='nearest')
       #idex=wind.index.get_loc(datetime.strptime(time_str[0:15],"%Y-%m-%d"))
       idex=wind.index.get_indexer([pd.to_datetime(time_str)], method='nearest')[0]
    ...
    if len(slons)!=0:
        ...
        for jj in range(len(stemp)):
            ax.text(x1[jj],y1[jj],str.format('{0:.1f}',float(stemp[jj])*1.8+32),color='w',fontsize=10,fontweight='bold',horizontalalignment='center',verticalalignment='center')
    ...
    #print(time_str+ ' before dtthis is defined')
    ...
    if (model=='DOPPIO') or (model=='GOMOFS'):
        ...
        plt.tricontourf(tri, temp,levels=clevs,shading='faceted',cmap=plt.cm.inferno,zorder=0)
        axis(gb)
        
        gca().patch.set_facecolor('0.5')
        cbar=colorbar()
        plt.tricontour(tri, depth,levels=dept_clevs,colors='white',zorder=10) # depth contour
    ...
    df=pd.read_csv('https://studentdrifters.org/tracks/drift_'+cluster+'.csv')
    ...
    for k in ids:
        ...
        datett=[]
        df1=df[df['ID']==k]
        df1=df1[df1['ID']>=240000000] # added this to keep only this years tracks
        ...
        for kk in range(len(df1)-1):
            ...
            if k==254410701:
                ...
                col='c'
            plt.plot([x1,x2],[y1,y2],col,linewidth=kk/(len(df1)/4))#markersize=kk)#,linewidth=kk)
    
    #if sys.argv[1]=='drift_whs_2022_1': # here is where we are adding miniboat track from Cassie
    #include_miniboats='yes'
    '''
    df=pd.read_csv('https://educationalpassages.org/wp-content/uploads/csv/MillenniumRaider_1.csv')
    df1=pd.DataFrame()
    ...
    for kk in range(len(df1)-1):
            x1,y1=df1['LON'].values[kk],df1['LAT'].values[kk]
            x2,y2=df1['LON'].values[kk+1],df1['LAT'].values[kk+1]
            plt.plot([x1,x2],[y1,y2],'r',linewidth=kk/(len(df1)/4))#markersize=kk)#,linewidth=kk)
    '''
    # This is where we read & plot makerbuoy data
    df=pd.read_csv('/home/user/drift/data/EPMBD_096_01.csv')
    #t1=c2f(df['buoy_temp_F'].values)[0]
    df1=pd.DataFrame()
    df1['datet']=pd.to_datetime(df['time_stamp'])
    df1.set_index('datet', inplace=True)
    df1=df1.tz_convert(None)
    df1['LON']=df['LON'].values
    df1['LAT']=df['LAT'].values
    dt64this = np.datetime64(dtthis)#.replace(tzinfo=pytz.utc))
    df1=df1[df1.index<dt64this]
    dt64this2=np.datetime64(dtthis-timedelta(days=2.0))
    df1=df1[df1.index>dt64this2]
    for kk in range(len(df1)-1):
            x1,y1=df1['LON'].values[kk],df1['LAT'].values[kk]
            x2,y2=df1['LON'].values[kk+1],df1['LAT'].values[kk+1]
            plt.plot([x1,x2],[y1,y2],'b',linewidth=kk/(len(df1)/4))#markersize=kk)#,linewidth=kk)
    df=pd.read_csv('/home/user/drift/data/EPMBD_097_01.csv')
    #t1=c2f(df['buoy_temp_F'].values)[0]
    df1=pd.DataFrame()
    df1['datet']=pd.to_datetime(df['time_stamp'])
    df1.set_index('datet', inplace=True)
    ...
    plt.title('drifters (green), MakerBuoys (blue),  miniboats (cyan), '+model+' '+layer+' model temp (color),\n wind (teal), depth 30&50m (white lines), eMOLT bottom temp (white #s), surface current (black)',fontsize=8,fontweight='bold')
    if detide=='y':
...
def make_images(dpath,path,dt=datetime(2019,5,1,0,0,0),interval=31,area='OOI',clevs=[39.,44.,0.5],lat_w=42.5,lon_w=-70.3,wind=pd.DataFrame([[datetime.now()-timedelta(1),datetime.now()],[.1,.1],[.1,.1]]),ooi=pd.DataFrame([[datetime.now()-timedelta(1),datetime.now()],[.1,.1],[.1,.1]]),model=model,detide='n',ooiw=pd.DataFrame([[datetime.now()-timedelta(1),datetime.now()],[.1,.1],[.1,.1]])):
    ...
    for j in range(interval):
        ...
        time_str=ntime.strftime('%Y-%m-%d %H%M')
        ...
        Day=str(ntime.day)
        Dayplusone=str((ntime+timedelta(days=1)).day)
        Monthplusone=str((ntime+timedelta(days=1)).month)
        slons,slats,stemp=[],[],[]
        try:
            '''
            slons,slats,stemp=[],[],[]
            for i in telemetered_dict[Year][Month][Day].index:
                ...
                '''
            url='https://erddap.emolt.net/erddap/tabledap/eMOLT_RT_QAQC.csvp?tow_id%2Csegment_type%2Ctime%2Clatitude%2Clongitude%2Cdepth%2Ctemperature%2Csensor_type&time%3E='+Year+'-'+Month+'-'+Day+'T00%3A00%3A00Z&time%3C'+Year+'-'+Monthplusone+'-'+Dayplusone+'T00%3A00%3A00Z'
            ...
            dfedr=dfedr.reset_index()
            for i in range(len(dfedr)):
                slons.append(dfedr['lon'][i])
                slats.append(dfedr['lat'][i])
                stemp.append(dfedr['mean_temp'][i])
...
def make_gif(gif_name,png_dir,start_time=False,end_time=False,frame_length = 0.2,end_pause = 4 ):
    ...
    imageio.mimsave(gif_name, images,'GIF',loop=0,duration=frame_length)
